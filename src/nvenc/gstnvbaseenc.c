...
/*    CASE (BD, BD);*/
      ...
      // Conductor: technical bug? 
      // CASE (LOW_LATENCY_HP, LOW_LATENCY_HQ);
      CASE (LOW_LATENCY_HP, LOW_LATENCY_HP);
...
gst_nv_base_enc_set_format (GstVideoEncoder * enc, GstVideoCodecState * state)
{
  ...
  if (!old_state) {
    ...
    // Conductor: проблема с синхронной записью от 3-х камер
    // https://habr.com/ru/post/262563/
    // "Эффективное кодирование видео в Linux c Nvidia NVENC" 
    ...
    // Не помогло - прошито nvidia на уровне драйверов
    // будем раскидывать на две видеокарты
